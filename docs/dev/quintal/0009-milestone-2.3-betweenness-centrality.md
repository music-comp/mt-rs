...
Compute betweenness centrality for all chords using Brandes' algorithm; identify the 6 saddle chords.

### Files to create

- `mt/src/quintal/centrality.rs` — Brandes' algorithm, saddle identification (~200 lines)
...
**`saddle_chords(space)`** -- top 6 chords by betweenness centrality.
...
|------|-------------|----------|
| `test_saddle_count` | `saddle_chords(space).len()` | 6 |
| `test_saddle_are_q686` | all saddle chords have IS (6,8,6) | true |
| `test_saddle_centrality_approx` | each saddle ~13.9% betweenness | within tolerance |
...
| `test_saddle_includes_known` | [0,2,6,8] is a saddle chord | true |
...
**Expected result:** The 6 [d5,A5,d5] chords (Q686 orbit) each have ~13.9% betweenness centrality, making them the dominant saddle of the space. These are: {0,2,6,8}, {1,3,7,9}, {2,4,8,10}, {3,5,9,11}, {4,6,10,0}, {5,7,11,1}.
...
**Saddle significance (paper section 7):** The saddle chords are the most structurally important in the entire space -- every shortest path tends to pass through them. They sit at the intersection of major voice-leading highways in the quintal space.
...
- **6** saddle chords, all Q686 orbit members
- Each saddle chord has approximately **13.9%** (0.139) betweenness centrality
...
- The 6 saddle chords are: **{0,2,6,8}** and its 5 transpositions by 2 semitones each (the Q686 orbit has size 6)
- Centrality is computed for all **228** chords
- Saddle chords have **maximum** betweenness among all 228 chords
