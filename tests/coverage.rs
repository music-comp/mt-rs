...
// ============================================================
// CLI binary integration tests (cover bin/rustmt.rs)
// ============================================================

#[test]
fn test_cli_scale_c_major() {
    let output = std::process::Command::new("cargo")
        .args(["run", "--", "scale", "C", "major"])
        .output()
        .expect("Failed to run CLI");
    let stdout = String::from_utf8_lossy(&output.stdout);
    assert!(stdout.contains("C"), "Output: {}", stdout);
    assert!(stdout.contains("E"));
    assert!(stdout.contains("G"));
}

#[test]
fn test_cli_chord_c_major() {
    let output = std::process::Command::new("cargo")
        .args(["run", "--", "chord", "C", "Major"])
        .output()
        .expect("Failed to run CLI");
    let stdout = String::from_utf8_lossy(&output.stdout);
...
#[test]
fn test_cli_scale_descending() {
    let output = std::process::Command::new("cargo")
        .args(["run", "--", "scale", "--descending", "C", "major"])
        .output()
        .expect("Failed to run CLI");
