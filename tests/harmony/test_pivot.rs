...
mod pivot_tests {
    ...
    fn test_c_major_to_g_major() {
        ...
        assert!(pivots.iter().any(|p|
            p.roman_in_a.degree == 5 && p.roman_in_b.degree == 1
        ));
        // C major is I in C and IV in G
        assert!(pivots.iter().any(|p|
            p.roman_in_a.degree == 1 && p.roman_in_b.degree == 4
        ));
    }
    ...
    fn test_distant_keys_fewer_pivots() {
        let close = harmony::pivot_chords(
            Pitch::from(C), Mode::Ionian,
            Pitch::from(G), Mode::Ionian,
        );
        let distant = harmony::pivot_chords(
            Pitch::from(C), Mode::Ionian,
            Pitch::from(Fs), Mode::Ionian,
        );
