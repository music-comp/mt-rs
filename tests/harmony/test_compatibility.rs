...
mod compatibility_tests {
    ...
    fn test_c_major_triad_compatible_scales() {
        ...
        assert!(scales.iter().any(|(p, m)| p.as_u8() == 0 && *m == Mode::Ionian));
        assert!(scales.iter().any(|(p, m)| p.as_u8() == 5 && *m == Mode::Lydian));
    }
    ...
    fn test_dm7_compatible_scales() {
        ...
        let scales = harmony::compatible_scales(&chord);
        assert!(scales.iter().any(|(p, m)| p.as_u8() == 2 && *m == Mode::Dorian));
    }
