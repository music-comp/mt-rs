...
mod exotic_scale_tests {
    ...
    fn test_modes_in_flat_keys() {
        ...
        for (tonic, mode, expected_count) in test_cases {
            let scale =
                Scale::new(ScaleType::Diatonic, tonic, 4, mode, Direction::Ascending).unwrap();
            let notes = scale.notes();
            assert_eq!(
                notes.len(),
                expected_count + 4, // 8 notes total
                "Scale {:?} should have 8 notes",
                scale
            );
        }
    }
    ...
    fn test_scale_context_consistency() {
        ...
        for (tonic, mode) in test_cases {
            ...
            assert_eq!(
                unique_letters.len(),
                7,
                "Scale {:?} should use each letter name exactly once in its 7 unique pitches",
                scale
            );
        }
    }
    ...
    fn test_complex_key_signatures() {
        ...
        let test_cases = vec![
            ...
            (
                ...
                Some(Mode::Ionian),
                vec![Cs, Ds, Es, Fs, Gs, As, Bs, Cs],
            ),
            // Gb major scale (6 flats)
            (
                Pitch::new(NoteLetter::G, -1),
                Some(Mode::Ionian),
                vec![Gb, Ab, Bb, Cb, Db, Eb, F, Gb],
            ),
            // F# major scale (6 sharps)
            (
                Pitch::new(NoteLetter::F, 1),
                Some(Mode::Ionian),
                vec![Fs, Gs, As, B, Cs, Ds, Es, Fs],
            ),
        ];
